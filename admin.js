...
function renderSummary(overview) {
  ...
  addSummaryItem("Prenotazioni attive", String(activeReservations.length));
...
function renderReservations(reservations) {
  ...
  document.querySelector("#bookingCount").textContent = reservations.length + (reservations.length === 1 ? " prenotazione" : " prenotazioni");
  if (!reservations.length) {
    const empty = document.createElement("p");
    empty.className = "reservation-empty";
    empty.textContent = "Nessuna prenotazione per questa data.";
