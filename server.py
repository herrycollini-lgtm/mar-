...
class MareHandler(SimpleHTTPRequestHandler):
    ...
    def end_headers(self):
        ...
        self.send_header("Content-Security-Policy", "default-src 'self'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self'; connect-src 'self'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'")
        super().end_headers()
    ...
    def static_path_allowed(self, path):
        if path in {"/", "/index.html", "/admin.html", "/styles.css", "/app.js", "/admin.js"}:
            return True
        return bool(re.fullmatch(r"/assets/[A-Za-z0-9_.-]+\.jpg", path, flags=re.IGNORECASE))
